...
import { Trophy, Loader2, Trash2, MapPin, Plus, Pencil, Check, X, Clock } from 'lucide-react'
...
export function TournamentPage() {
  ...
  const { tournament, categories, matches, teamsMap, venues, loading, error, fetchTournament, generateKnockouts, generateFinals, deleteTournament, addVenue, editVenue, removeVenue, assignVenue, updateSchedule } = useTournamentStore()
    ...
    <div className="flex flex-col items-center min-h-dvh bg-hbl-bg p-4 gap-4">
          ...
          <div key={group.id} className="w-full max-w-lg flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              {sortByStartsAt(grpMatches).map((match) => (
                <MatchRow key={match.id} match={match} teamsMap={teamsMap} isAdmin={isAdmin} venues={venues} onAssignVenue={assignVenue} onUpdateSchedule={updateSchedule} />
              ))}
            </div>
          </div>
        )
      ...
      {hasKnockouts && (
        <div className="w-full max-w-lg">
          <BracketView matches={sortedKnockoutMatches} teamsMap={teamsMap} isAdmin={isAdmin} />
        </div>
      )}
    </div>
  )
...
function MatchRow({ match, teamsMap, isAdmin, venues, onAssignVenue, onUpdateSchedule }: {
  match: DbMatch
  teamsMap: Map<string, DbTeam>
  isAdmin: boolean
  venues: DbVenue[]
  ...
  return (
    <div className="flex flex-col gap-1">
      <div
        onClick={() => { if (!isFinished && isAdmin) setEditing(true) }}
        className={`relative flex flex-col gap-1 px-3 py-2 rounded-lg bg-hbl-surface border border-hbl-border text-sm w-full text-left ${
          !isFinished && isAdmin ? 'hover:border-hbl-accent cursor-pointer' : isFinished ? 'opacity-80' : ''
        }`}
      >
        <div className="flex items-center gap-2">
