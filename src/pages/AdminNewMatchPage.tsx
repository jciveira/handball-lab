...
import { CodeRow, LinkRow } from '../components/ui/CodeRow'
...
function MatchCreated({ match, homeTeam, awayTeam }: { match: DbMatch; homeTeam: string; awayTeam: string }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh bg-hbl-bg p-6 gap-8">
      ...
      <div className="flex flex-col gap-4 w-full max-w-sm">
        <LinkRow label="Enlace de espectador" url={`${window.location.origin}/match/${match.id}`} hint="Compartir por WhatsApp — acceso directo" />
        <CodeRow label="Código de anotador" code={match.scorekeeper_code} hint="Operador de puntuación y reloj" />
        <CodeRow label="Código de estadísticas" code={match.stat_tracker_code} hint="Operador de estadísticas" />
      </div>
      ...
      <p className="text-xs text-hbl-text-muted/50 mt-2">
        Comparte el enlace con los espectadores y los códigos con los operadores.
      </p>
    </div>
  )
}
