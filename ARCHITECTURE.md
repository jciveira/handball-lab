...
| Auth | Code-based access (viewers, operators) + 1 admin account | Simple enough for parents/kids, secure enough for control |
...
Scorekeeper (1 per match, code-based)
  - Claim role with match-specific code (first come, first served)
  - Controls: score +/-, clock, exclusions, timeouts, half-time

Stat Tracker (1 per match, code-based)
  - Claim role with match-specific code (first come, first served)
...
handball-lab/
  src/
    ...
    lib/
      ...
      auth.ts             # Code-based access logic
