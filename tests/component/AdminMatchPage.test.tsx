...
describe('AdminMatchPage', () => {
  ...
  it('shows match info with team names and scorekeeper code', async () => {
    ...
    expect(screen.getByText('SK1234')).toBeInTheDocument()
    expect(screen.getByText('Código de anotador')).toBeInTheDocument()
  })

  it('shows viewer link with match URL', async () => {
    mockGetMatch.mockResolvedValue(MATCH)
    mockTeamLookup()
    renderPage()

    await waitFor(() => {
      expect(screen.getByText('Enlace de espectador')).toBeInTheDocument()
    })
